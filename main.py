...
def main(separate_sentences_analysis, nb_comments, compute_freq_items, compute_freq_pairs):
    ...
    for nb_clusters in [5, 15]:
        ...
        log = open("results\clusters_" + str(nb_clusters) +
                   "_" + str(nb_comments) + ".tsv", "w")

        for name, algorithm in clustering_algorithms:
            if not os.path.exists("results/"+name + "_" + str(nb_clusters) + "_" + str(nb_comments)):
                os.mkdir("results/"+name + "_" +
                         str(nb_clusters) + "_" + str(nb_comments))
                print("Directory ", name, " Created ")
            else:
                print("Directory ", name, " already exists")
            # write the name of algorithm on top of data in record file
            log.write("Hotel Name" + "\t" + "reviews" +
                      "\t" + "time" + "\t" + name + "\n")
            ...
            for Hotel_name in unique_hotels:
                ...
                Number_of_Reviews = list(
                    data.Total_Number_of_Reviews[data.Hotel_Name == Hotel_name])[0]
                ...
                if Number_of_Reviews > nb_comments:
                    ...
                    print("Number of reviews : " +
                          str(Number_of_Reviews) + " for Hotel : " + Hotel_name)
                    log.write(Hotel_name + "\t" +
                              str(Number_of_Reviews) + "\t")

                    # measuring time for each cluster
                    time_start_clustering = time.clock()
                    Neg_reviews.clusters = cluster_sentences(
                        Negative_Reviews, algorithm)
                    Pos_reviews.clusters = cluster_sentences(
                        Positive_Reviews, algorithm)
                    time_elapsed_clustering = (
                        time.clock() - time_start_clustering)
                    ...
                    common_words_count = compute_algorithm_score(
                        Pos_reviews, Neg_reviews)

                    save_important_sentences(Neg_reviews, Pos_reviews)

                    # writing the results in record file
                    write_results(Pos_reviews, Neg_reviews, name, nb_clusters, nb_comments,
                                  hotel_count, Hotel_name, compute_freq_items, compute_freq_pairs)
                    ...
                    algorithm_file = open(
                        "results/"+name + "_" + str(nb_clusters) + "_" + str(nb_comments) + "/" + "report.txt", "w")
                    algorithm_file.write(
                        "Number of reviews : " + str(Number_of_Reviews) + " for Hotel : " + Hotel_name + "\n")
                    algorithm_file.write("time : " + str(time_elapsed) + "\n")
                    algorithm_file.write(
                        "number of common words between clusters : " + str(common_words_count) + "\n")
                    log.write(str(common_words_count /
                              (nb_clusters*(nb_clusters-1)*5)) + "\n")
...
if __name__ == "__main__":
    ...
    main(separate_sentences_analysis, nb_comments,
         compute_freq_items, compute_freq_pairs)
